...
describe("Test", () => {
  ...
  it("opens multiple independent positions at overlapped tick positions", async () => {
    const ranges = [
      { lower: 0, upper: 19 },
      { lower: 15, upper: 35 },
      { lower: 30, upper: 59 },
    ...
    for (const range of ranges) {
      ...
      totalLiq = totalLiq.add(new BN(liq.toString()));
    }

    const pool = await program.account.poolState.fetch(pool_state_pda);

    // At tick=0, only position [0,19] is active (its range contains tick 0)
    const firstRangeLiq = new BN(
      Math.floor(
        15 * (10 ** 9) * (sqrtPrice(19) * sqrtPrice(0))
          / (sqrtPrice(19) - sqrtPrice(0))
      ).toString()
    );
    assert.ok(
      pool.liquidity.eq(firstRangeLiq),
      `Active liquidity at tick 0 should equal first position's liq (${firstRangeLiq}), got ${pool.liquidity}`
    );
    assert.strictEqual(pool.currentTick, 0, "Pool tick should remain at 0 after opening positions above it");
  });

  it("increases liquidity then decreases it back to original", async () => {
    const extraLiq = new BN(10_000_000_000);

    const poolBefore = await program.account.poolState.fetch(pool_state_pda);
    const liqBefore = poolBefore.liquidity;

    // Increase
    const txIncrease = await program.methods
      .increaseLiquidity(
        extraLiq,
        new BN(100_000_000),
        new BN(100_000_000),
      ...
      .rpc();
    await program.provider.connection.confirmTransaction(txIncrease);

    const poolMid = await program.account.poolState.fetch(pool_state_pda);
    assert.ok(
      poolMid.liquidity.eq(liqBefore.add(extraLiq)),
      `Liquidity after increase should be ${liqBefore.add(extraLiq)}, got ${poolMid.liquidity}`
    );

    // Decrease back
    const txDecrease = await program.methods
      .decreaseLiquidity(
        extraLiq,
        new BN(0),
        new BN(0),
      ...
      .rpc();
    await program.provider.connection.confirmTransaction(txDecrease);

    const poolAfter = await program.account.poolState.fetch(pool_state_pda);
    assert.ok(
      poolAfter.liquidity.eq(liqBefore),
      `Liquidity after decrease should return to ${liqBefore}, got ${poolAfter.liquidity}`
    );
  });
  ...
  it("swaps token_1 (B) for token_0 (A) staying in range", async () => {
    ...
    // Small swap should not change the active liquidity (still within [0,19])
    assert.ok(
      poolAfter.liquidity.eq(liqBefore),
      `Liquidity should remain ${liqBefore} after small in-range swap, got ${poolAfter.liquidity}`
    );
    // Price should have moved slightly (sqrtPriceX64 increased for B→A swap)
    assert.ok(
      poolAfter.sqrtPriceX64.gt(poolBefore.sqrtPriceX64),
      "Price should increase after swapping B for A"
    );
    // Tick should still be 0 (swap is small)
    assert.strictEqual(poolAfter.currentTick, tickBefore, "Tick should remain at 0 for small swap");
  ...
  it("swaps token_1 (B) for token_0 (A) hopping across multiple ticks", async () => {
    ...
    // The price limit was tick 59, so the swap should reach or approach it
    assert.ok(
      poolAfter.currentTick >= POSITION_TICK_UPPER,
      `Current tick ${poolAfter.currentTick} should have crossed upper 59`
    );

    // At tick >= 59, all three positions have been exited
    // [0,19] exited at tick 19, [15,35] exited at tick 35, [30,59] exited at tick 59
    assert.ok(
